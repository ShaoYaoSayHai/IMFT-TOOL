...
//    return GT_Modbus::ERROR;
  ...
  if (data.at(1) == 0x03) {
    ...
    if (regAddress == 0xA0A0) {
      ...
      emit sig_updateAirPressure(slaveAddr, AirPressureLocation::AIR, value);
    } else if (regAddress == 0x4010) { /* 读取进气端压力 */
      int value = ParseGtAirPressureRead(qbyData);
      emit sig_updateAirPressure(slaveAddr, AirPressureLocation::INF, value);
    } else if (regAddress == 0x4012) { /* 读取出气端压力 */
      int value = ParseGtAirPressureRead(qbyData);
      emit sig_updateAirPressure(slaveAddr, AirPressureLocation::END, value);
      ...
      emit sig_updateValveStatus(slaveAddr,
