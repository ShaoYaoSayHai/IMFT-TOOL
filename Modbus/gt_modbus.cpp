...
void GT_Modbus::GT_SetSlaveAddr(uint8_t slaveID)
{
    this->slaveAddr = slaveID ;
}

uint8_t GT_Modbus::GT_GetSlaveAddr()
{
    return this->slaveAddr ;
}


QByteArray GT_Modbus::GT_ModbusWrite(uint8_t slaveAddr, uint8_t funcCode, uint16_t regAddr, uint8_t *regValue, uint8_t regSize, ModbusReMappingFunc func)
{
...
int GT_Modbus::GT_RetMsgVerify(QByteArray data)
{
    ...
    uint16_t regAddress =( (uint8_t)(data.at(2)) << 8 ) | ( (uint8_t)(data.at(3)) & 0xFF ) ;
...
//    }
    ...
    if( data.at(1) == 0x03 )
    {
        ...
        /* 读取大气压 */
        else if( regAddress == 0x4001 )
        {
            int value = ParseGtAirPressureRead( qbyData );
            // 读取到的压力值
            emit sig_updateAirPressure( AirPressureLocation::AIR , value );
        }
        else if( regAddress == 0x4010 )
        {
            int value = ParseGtAirPressureRead( qbyData ) ;
            emit sig_updateAirPressure( AirPressureLocation::INF , value );
        }
        else if( regAddress == 0x4012 )
        {
            int value = ParseGtAirPressureRead( qbyData ) ;
            emit sig_updateAirPressure( AirPressureLocation::END , value );
        }
        else if( regAddress == 0x4045 )
        {
            qDebug()<<qbyData ;
