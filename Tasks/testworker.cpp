...
{
    // 线程启动/结束的槽
    connect(&testWorkerThread, &QThread::started,
            this, &TestWorker::onThreadStarted);
    connect(&testWorkerThread, &QThread::finished,
            this, &TestWorker::onThreadFinished);
}
...
void TestWorker::startWorker()
{
    ...
    // 1. 通知 TestLoop 开启定时器
    if (pxTestLoop) {
        QMetaObject::invokeMethod(pxTestLoop, "TestTaskInit",
                                  Qt::QueuedConnection);
    }
}
