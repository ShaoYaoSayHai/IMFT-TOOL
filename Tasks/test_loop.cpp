...
/**
 * @brief TestLoop::LST_CommandOpenInputSwitch
 *        打开进气端阀门
 */
void TestLoop::LST_CommandOpenInputSwitch() {
  for (int i = 0; i < commandList.size(); i++) {
...
void TestLoop::DO_TaskOpenFire(QList<DeviceInfo> data) {
  ...
  emit logCurrentStep(
      "========================= 点火开阀测试 BEGIN =========================");
  // 执行2KPa阀门开启 发送打开阀门指令
  LST_CommandOpenInputSwitch();
  // 此时的管道供气2KPa，然后等待几秒钟时间，在这几秒钟准备执行清除异常
  for (int i = 0; i < 3; i++) {
    emit logCurrentStep("等待压力稳定 " + QByteArray::number(5 - i));
