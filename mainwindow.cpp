...
void MainWindow::SerialWorkerInit()
{
    ...
    // 连接信号
    connect(pxSerialWorkerUART_Handler, &SerialWorker::sig_connectStatus,
            this, &MainWindow::onConnectStatusChanged);
    connect(pxSerialWorkerUART_Handler, &SerialWorker::sig_readData,
            this, &MainWindow::onSerialWorkerReadData);

    RefreshSerialPorts(); // 刷新串口

    connect( pxTimerReadSN , &QTimer::timeout , this , &MainWindow::onTimerTimeoutReadSN);
}
...
//    ui->pushButton_6->setVisible(false) ;
    ...
    connect(&GT_ModbusHandler, &GT_Modbus::sig_updateAirPressure,
            this, [=](int location, int value) {
        // 在这里直接处理大气压值更新逻辑
        qDebug() << "位置:" << location << "大气压值:" << value;

    });
    connect(&GT_ModbusHandler, &GT_Modbus::sig_updateSN,
...
{
    //    QByteArray qbyData = GT_ModbusHandler.GT_ModbusWrite( 0xAA , 0x03 , 0xA0A0 , NULL , 0 , NULL ) ;
    QByteArray qbyData = GT_ModbusHandler.GT_ModbusWrite( 0x0A , 0x03 , 0x4012 , NULL , 0 , NULL ) ;
    HexPrintf(qbyData) ;
    pxSerialWorkerUART_Handler->writeData( qbyData ) ;
}
