...
void MainWindow::GUI_TableInit() {
   ...
   */
    ...
    connect(&(pxTestWorkerHandler->pxTestLoop->GT_ModbusHandler),
        ...
        // 2) 计算末尾两位（严格两字节/两字符语义）
        QByteArray last2;
        if (sn.size() >= 2) {
            last2 = sn.right(2);
        } else {
            last2 = sn; // 或者 last2.clear();
        }

        // 4) 追加到列表
        DeviceInfo device;
        device.SN = sn;
        device.slaveID = last2;


        QTableWidget* w = pxTable->GetTableWidget();
        const QString last2Str = QString::fromLatin1(last2);
        // 查询是否已经存在过
        const int existRow = findFirstColumnByLast2( w , last2Str  ) ;
        if( existRow != -1 )
        {
            // 当前的已经存在过
            ...
            // 可选：高亮已有行，提示用户
            // 例如把该行第一列背景改红（需要你有 SetCellItem 或直接操作 item）
            // QTableWidgetItem* item = w->item(existRow, 0);
            // if (item) item->setBackground(QColor(255, 0, 0));
        ...
        // 判断是否存在，如不存在才添加
        GT_DeviceList.append(device);
        // 5) 确保表格行数足够（你的 SetCellItem 是 1-based）
        const int row1 = GT_DeviceList.size();  // 1-based 行号

        // 如果你没有暴露底层 table 指针，就在 TableControl 里提供 EnsureRowCount(row1) 方法
        if (w && w->rowCount() < row1) {
            w->setRowCount(row1);
        }
...
void MainWindow::checkAllSwitchStatus()
{
    for( DeviceInfo &device : GT_DeviceList )
    {
        ...
        {
            pxTable->SetCellItem(
                        (findFirstColumnByLast2(ui->tableWidget, device.slaveID) + 1), 4,
                        "FAIL" ,  TableControl::RED);
        }
    }
    ResetButtonEnable();
